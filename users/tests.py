    ...
    def test_username_validation(self):
        ...
        for username in valid_usernames:
            with self.subTest(username=username):
                ...
                user.full_clean()  # Should not raise validation error

        invalid_usernames = ['123user', 'user@name', 'user name', 'ab', 'a' * 21]
...
class UserAdminTestCase(APITestCase):
    ...
    def test_toggle_admin_status(self):
        ...
        response = self.client.post(f'/api/users/{self.user.id}/toggle-admin/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
